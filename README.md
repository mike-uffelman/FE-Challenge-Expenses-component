...
On the surface this component is pretty simple, however to spice it up a bit, I created a generator function with Fakerjs to create additional expenses over many months. The additional data points result in greater functionality for the user and allowed for additional practice using more React features including useState, useEffect, and useRef.

The transaction data provided by the challenge was rather basic, so to add complexity and more substance to the transactions the data generator includes an ID, date, vendor, amount, and category of the expense. Not all of these are used in the current version of the component but could provide useful data for future versions. For example, clicking on a bar graph for a specific day could display all the transactions for that day with the vendor and product categories, which would provide valuable data for the user.
...
In the lower right corner of the UI component, the user can see the percent change for spending against the previous period. For example, if the component is showing seven (7) days of data, the change value is percent change from prior week to this week, i.e. ((current - previous)/previous) \* 100.
...
### Getting a prior period start date

The below code snippet gets the start date for the prior period by taking the current start date and subtracting the period length from it.

The current start date is passed inside the new Date() constructor, which allows for the setDate() method to be called. Inside setDate(), which changes the date value of the date object calling the method, we take the current start date and subtract the period length (daysBack) to get the prior period start date.
...
Now that we have a range for the entire date range we've queried, we can find the symmetric differences between all transactions and the desired date range.

Now that an array for the entire date range is available, the days with zero spending (i.e. do not exist in the transaction data) can be identified. The snippet below checks the transactions array for the date in the desired date range and if it does not exist, create a new object with properties of 'date' as the date missing from data and 'amount' of zero(0) and push the object to an array defined prior. This results in an array of the zero spend days.
...
And lastly, the array of combined zero spend days and transactions needs to be reduced to an object with each day as a key and an array of transactions as the value. This will allow for multiple transactions on a given day to be combined into a single date property. In the snippet below, the data is reduced down to a single object. Using the current date as a key to access the new object property, the function looks for the property with the given key (i.e. date), if it does not exist then assign an empty array as the value for the key. Then take the key property and push the current data index to the matching key property (i.e. date)
...
With this reduced data object, it can be passed into each component and render the appropriate information for the user.
